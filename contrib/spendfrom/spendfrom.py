...
#
# Assumes it will talk to a dashd or Fatzchain-Qt running
# on localhost.
#
# Depends on jsonrpc
#
...
def determine_db_dir():
    """Return the default location of the fatzchain data directory"""
    if platform.system() == "Darwin":
        return os.path.expanduser("~/Library/Application Support/Fatzchain/")
    elif platform.system() == "Windows":
        return os.path.join(os.environ['APPDATA'], "Fatzchain")
    return os.path.expanduser("~/.fatzchain")

def read_bitcoin_config(dbdir):
    """Read the fatzchain.conf file from dbdir, returns dictionary of settings"""
    ...
    config_parser.readfp(FakeSecHead(open(os.path.join(dbdir, "fatzchain.conf"))))
    return dict(config_parser.items("all"))

def connect_JSON(config):
    """Connect to a fatzchain JSON-RPC server"""
...
def list_available(dashd):
    ...
    for output in unspent:
        ...
        # This code only deals with ordinary pay-to-fatzchain-address
...
def main():
    ...
    parser.add_option("--datadir", dest="datadir", default=determine_db_dir(),
                      help="location of fatzchain.conf file with RPC username/password (default: %default)")
